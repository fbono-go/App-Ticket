// GO Mantenimiento — Service Worker v5 (notificaciones multicapa, sin tráfico duplicado)
...
function mostrarNotif(titulo, cuerpo, tag) {
  ...
  return self.registration.showNotification(titulo, {
    ...
    icon: base + 'icons/icon-go-mantenimiento-192.png',
    badge: base + 'icons/icon-go-mantenimiento-192.png',
    vibrate: [200, 100, 200],
    data: { url: base }
  });
}
...
self.addEventListener('message', async event => {
  ...
  if (type === 'TEST_NOTIF') {
    await mostrarNotif('✅ Notificación de prueba',
      'Las notificaciones de GO Mantenimiento funcionan correctamente', 'test_' + Date.now());
    if (reply) reply.postMessage({ ok:true });
  }
