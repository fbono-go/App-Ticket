...
(function () {
  ...
  function cargar() {
    ...
    Promise.all([
      pedir('/bandeja'),
      // las correcciones que pidieron los técnicos se revisan en el mismo lugar
      pedir('/cambios?estado=pendiente').catch(function () { return { cambios: [] }; }),
    ]).then(function (rs) {
      PENDIENTES = rs[0].pendientes || [];
      CAMBIOS = rs[1].cambios || [];
      pintar();
      pintarBadge();
    }).catch(function (e) {
      if (c) {
  ...
  function pintarBadge() {
    var b = $('ban-badge');
    if (!b) return;
    var total = PENDIENTES.length + CAMBIOS.length;
    b.textContent = total;
  ...
  /**
   * Las peticiones de los técnicos sobre un equipo, separadas en dos clases.
   *
   * Un pedido de baja no es "cambió la marca": es una decisión de otro peso.
   * Va primero, con otro color y otro verbo, para que nunca se confunda con
   * una corrección de dato entre muchas.
   */
  function esBaja(c) { return c.campo === 'estado_equipo' && c.valor_nuevo === 'fuera_servicio'; }

  function pintarCambios() {
    if (!CAMBIOS.length) return '';
    var bajas = CAMBIOS.filter(esBaja);
    var datos = CAMBIOS.filter(function (c) { return !esBaja(c); });

    return pintarBajas(bajas) + pintarCorrecciones(datos);
  }

  function pintarBajas(lista) {
    if (!lista.length) return '';
    return '<div class="ban-tit" style="font-size:15px;margin:4px 0 8px;color:#c0392b">' +
      '⚠ Piden dar de baja estos equipos</div>' +
      lista.map(function (c) {
        return '<div class="ban-caso ban-cambio ban-baja" data-cambio="' + esc(c.cambio_id) + '">' +
          '<div class="ban-cab"><span class="ban-id">' + esc(c.equipo_id) + '</span>' +
          '<span class="ban-fecha">' + esc(fechaCorta(c.fecha)) + '</span></div>' +
          '<div class="ban-ubi">' + esc(c.referencia) + '</div>' +
          '<div class="ban-quien">Lo pidió ' + esc(c.solicitado_por) + '</div>' +
          '<div class="ban-desvio" style="background:#fdecea;color:#8b291f">' +
            '<b>' + (/^Reemplazo:/.test(c.motivo || '') ? 'Piden reemplazarlo'
              : 'Piden darlo de baja') + '</b><br>' +
            esc((c.motivo || 'sin detalle').replace(/^(Reemplazo|Baja):\s*/, '')) + '</div>' +
          '<div class="ban-acc">' +
          '<button class="ban-btn" style="background:#c0392b;color:#fff" ' +
            'data-cacc="aprobado">Dar de baja</button>' +
          '<button class="ban-btn ban-no" data-cacc="rechazado">Mantener en servicio</button>' +
          '</div><div class="ban-msg" data-msg></div></div>';
      }).join('');
  }

  function pintarCorrecciones(lista) {
    if (!lista.length) return '';
    return '<div class="ban-tit" style="font-size:15px;margin:4px 0 8px">' +
      'Correcciones pedidas</div>' +
      lista.map(function (c) {
        return '<div class="ban-caso ban-cambio" data-cambio="' + esc(c.cambio_id) + '">' +
          '<div class="ban-cab"><span class="ban-id">' + esc(c.equipo_id) + '</span>' +
          '<span class="ban-fecha">' + esc(fechaCorta(c.fecha)) + '</span></div>' +
          '<div class="ban-ubi">' + esc(c.referencia) + '</div>' +
          '<div class="ban-quien">Lo pidió ' + esc(c.solicitado_por) + '</div>' +
          '<div class="ban-desvio" style="background:#eef3fb">' +
            '<b>' + esc(c.campo.replace(/_/g, ' ')) + '</b>: ' +
            esc(c.valor_anterior || '(vacío)') + ' → <b>' + esc(c.valor_nuevo) + '</b></div>' +
          (c.motivo ? '<div class="ban-ubi">' + esc(c.motivo) + '</div>' : '') +
          '<div class="ban-acc">' +
          '<button class="ban-btn ban-ok" data-cacc="aprobado">Aprobar</button>' +
          '<button class="ban-btn ban-no" data-cacc="rechazado">Rechazar</button>' +
          '</div><div class="ban-msg" data-msg></div></div>';
      }).join('');
  }

  document.addEventListener('click', function (e) {
    var b = e.target.closest ? e.target.closest('[data-cacc]') : null;
    if (!b) return;
    var caso = b.closest('.ban-cambio');
    if (!caso) return;

    var id = caso.getAttribute('data-cambio');
    var acc = b.getAttribute('data-cacc');
    var msg = caso.querySelector('[data-msg]');

    var motivo = '';
    if (acc === 'rechazado') {
      motivo = prompt('¿Por qué se rechaza? Queda registrado.');
      if (motivo === null || !motivo.trim()) return;
    }

    caso.querySelectorAll('button').forEach(function (x) { x.disabled = true; });
    if (msg) { msg.textContent = 'Guardando…'; msg.className = 'ban-msg on'; }

    pedir('/cambio/' + encodeURIComponent(id), {
      method: 'POST',
      body: { accion: acc, decidido_por: QUIEN(), motivo_decision: (motivo || '').trim() },
    }).then(function () {
      CAMBIOS = CAMBIOS.filter(function (x) { return x.cambio_id !== id; });
      if (caso.parentNode) caso.parentNode.removeChild(caso);
      pintarBadge();
      if (!PENDIENTES.length && !CAMBIOS.length) pintar();
      aviso(acc === 'aprobado' ? 'Cambio aplicado' : 'Cambio rechazado');
      setTimeout(cargar, 600);
    }).catch(function (err) {
      caso.querySelectorAll('button').forEach(function (x) { x.disabled = false; });
      if (msg) { msg.textContent = 'Error: ' + err.message; msg.className = 'ban-msg on mal'; }
    });
  });
  ...
  function confirmar(caso, dato) {
    ...
    var datos = {
      ...
      // corto y siempre igual: el detalle del desvío va en la descripción
      title: 'Correctivo de ' + dato.equipo_id,
  ...
  /**
   * Saca el caso de la lista.
   *
   * Se quita del DOM en el acto y además se vuelve a consultar el servidor.
   * Antes se esperaban dos segundos para removerlo y se confiaba en el estado
   * en memoria: si el usuario cambiaba de pestaña en el medio, o si algo del
   * encadenado fallaba, el caso quedaba visible aunque ya estuviera decidido.
   */
  function sacar(caso, dato, texto) {
    ...
    aviso(texto + ' · ' + dato.equipo_id);

    // el servidor es la verdad: se refresca por si hubo otra decisión en paralelo
    setTimeout(cargar, 600);
  }
  ...
  '.ban-cambio{border-left-color:#0060D6!important;}' +
  '.ban-baja{border-left-color:#c0392b!important;background:#fffaf9;}' +
  ...
  '#ban-badge{display:none;min-width:16px;height:16px;border-radius:8px;' +
  ...
  function montar() {
    ...
    if (nav) {
      ...
      // La barra del anfitrión es un grid. Fijar las columnas con un número
      // calculado al montar se rompe: el botón "Crear" aparece y desaparece
      // según el modo premium, y la barra queda con una columna de más o de
      // menos. Con flujo automático por columnas se acomoda sola, y los
      // botones ocultos no ocupan lugar.
  ...
  function arrancar() {
    montar();
    // se consulta al arrancar solo para marcar el badge: tiene que contar las
    // dos cosas que esperan revisión, no solo los correctivos
    Promise.all([
      pedir('/bandeja').catch(function () { return { pendientes: [] }; }),
      pedir('/cambios?estado=pendiente').catch(function () { return { cambios: [] }; }),
    ]).then(function (rs) {
      PENDIENTES = rs[0].pendientes || [];
      CAMBIOS = rs[1].cambios || [];
