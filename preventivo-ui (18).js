...
(function () {
  ...
  /**
   * Estado de vencimiento a partir de los días que faltan. Es el único lugar
   * donde se decide el color: el servidor manda los días, la app los clasifica.
   ...
   *   gris     → nunca se le hizo un preventivo (no hay fecha para calcular)
   */
  ...
  function textoVenc(dias) {
    if (dias === null || dias === undefined) return 'Sin preventivo todavía';
   ...
   * proveedor u oficial, agrupado por lugar (el técnico va a un edificio y hace
   * lo que hay ahí) y del más urgente al menos urgente.
   */
  async function renderVencimientos(instId) {
    ...
    try {
      const r = await api('/semaforo?institucion=' + encodeURIComponent(instId) +
                          '&responsable=' + encodeURIComponent(FILTRO()));
      // `atencion` trae lo que vence dentro de 30 días, lo vencido y lo sin registro.
      // Lo que falta más de 30 días viene solo contado, en totales.verde.
      const lista = (r.atencion || []).map((e) =>
        Object.assign({}, e, { _est: estadoVenc(e.dias_restantes) }));

      const cuenta = { rojo: 0, naranja: 0, amarillo: 0, verde: (r.totales && r.totales.verde) || 0 };
      lista.forEach((e) => { if (e._est in cuenta) cuenta[e._est]++; });

      const contadores = `
        <div class="prev-cont">
          <div class="prev-cont-i rojo"><b>${cuenta.rojo}</b><span>Vencidos</span></div>
          <div class="prev-cont-i naranja"><b>${cuenta.naranja}</b><span>≤ 7 días</span></div>
          <div class="prev-cont-i amarillo"><b>${cuenta.amarillo}</b><span>8 a 14 días</span></div>
          <div class="prev-cont-i verde"><b>${cuenta.verde}</b><span>Al día</span></div>
        </div>`;

      if (!lista.length) {
        c.innerHTML = contadores + `<div class="prev-card prev-ok"><b>Todo al día</b>
          <p>No hay equipos vencidos ni por vencer en los próximos 30 días.</p></div>`;
        return;
      }

      // Los adelantables vienen por zona (lugar + piso); acá se agrupa solo por lugar.
      const adel = {};
      (r.zonas || []).forEach((z) => {
        const k = z.lugar || nombreLugar(instId, z.lugar_id);
        adel[k] = (adel[k] || 0) + (z.adelantables || 0);
      });

      const grupos = new Map();
      lista.forEach((e) => {
        const k = e.lugar || nombreLugar(instId, e.lugar_id) || 'Sin lugar';
        if (!grupos.has(k)) grupos.set(k, []);
        grupos.get(k).push(e);
      });
      const peor = (arr) => Math.min(...arr.map((e) => ORDEN_ESTADO[e._est]));
      const lugares = [...grupos.keys()].sort((a, b) =>
        (peor(grupos.get(a)) - peor(grupos.get(b))) || a.localeCompare(b));

      c.innerHTML = contadores + lugares.map((k) => {
        const items = grupos.get(k); // ya vienen ordenados por días desde el servidor
        return `
          <div class="prev-lugar">
            <i class="ti ti-map-pin"></i>${esc(k)}
            <span class="prev-lugar-n">${items.length}</span>
            ${adel[k] ? `<span class="prev-lugar-adel">+${adel[k]} se pueden adelantar</span>` : ''}
          </div>
          ${items.map((e) => `
            <div class="prev-venc ${e._est}">
              <div class="prev-venc-cab">
                <span class="prev-venc-est">${esc(textoVenc(e.dias_restantes))}</span>
                <span class="prev-venc-id">${esc(e.equipo_id)}</span>
              </div>
              <div class="prev-venc-tit">${esc([e.marca, e.tipo].filter(Boolean).join(' · ') || e.equipo_id)}</div>
              <div class="prev-venc-ubi">${esc([e.piso ? 'Piso ' + e.piso : '', e.ubicacion_detalle]
  ...
  .prev-cont{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin:16px 0 4px;}
  .prev-cont-i{background:#fff;border:1px solid #e3e8ef;border-radius:10px;padding:10px 4px;
    text-align:center;}
  .prev-cont-i b{display:block;font-size:20px;font-weight:800;line-height:1.1;}
  .prev-cont-i span{display:block;font-size:10.5px;font-weight:600;color:#8a93a0;margin-top:3px;}
  ...
  .prev-lugar{display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin:18px 2px 8px;
  ...
  .prev-venc-id{font-size:11px;color:#8a93a0;font-family:ui-monospace,Menlo,monospace;}
  .prev-venc-tit{font-size:14px;font-weight:700;margin-top:6px;}
  .prev-venc-ubi{font-size:12.5px;color:#5a6472;margin-top:2px;}
