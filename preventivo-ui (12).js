...
(function () {
  ...
  async function resolverCodigo(texto) {
    ...
    if (eq) {
      return modoEscaneo === 'trabajo' ? abrirTrabajo(eq) : abrirChecklist(eq);
    }
    if (modoEscaneo === 'trabajo') {
  ...
  function abrirAlta(id) {
    ...
    // El código de la etiqueta ya dice qué es: ICR-AE-0003 es una condensadora.
    // Preseleccionarlo evita el error de cargar un equipo con el tipo de otro,
    // que después hay que corregir a mano en la planilla.
    const prefijo = (String(id).split('-')[1] || '').toUpperCase();
    const coincide = tiposAlta.find((t) => String(t.prefijo_qr).toUpperCase() === prefijo);
    const sugerido = coincide ? coincide.tipo_id : '';
    ...
    $('prev-alta-cuerpo').innerHTML = `
      ...
      <select id="prev-a-tipo" class="prev-input" onchange="PREV.altaTipoCambio()">
        ${tiposAlta.map((t) => `<option value="${esc(t.tipo_id)}" ${
          t.tipo_id === sugerido ? 'selected' : ''}>${esc(t.nombre || t.tipo_id)}</option>`).join('')}
      </select>
      ${sugerido ? `<div class="prev-hint">Se eligió por el código de la etiqueta
        (<b>${esc(prefijo)}</b>). Cambialo si no corresponde.</div>` : ''}
    ...
    try {
      ...
      c.innerHTML = `
        <div class="prev-ficha">
          ...
          <div class="prev-ficha-sub">${esc([eq.lugar, eq.piso ? 'piso ' + eq.piso : '']
            .filter(Boolean).join(' · '))}</div>
          <div class="prev-ficha-sub">${r.total} registro${r.total === 1 ? '' : 's'}</div>
        </div>` +
        (r.eventos || []).map((e) => {
  ...
  // 'preventivo' (lo habitual) o 'trabajo'. El escáner es el mismo; cambia
  // adónde va después de reconocer el equipo.
  let modoEscaneo = 'preventivo';

  async function escanear(modo) {
    if (!cfg.institucion) { aviso('Elegí primero la institución'); return; }
    modoEscaneo = modo === 'trabajo' ? 'trabajo' : 'preventivo';
