...
(function () {
  ...
  async function renderInicio() {
    ...
    cont.innerHTML = `
      ...
      <button class="prev-btn prev-sec" onclick="PREV.verVencimientos()" ${inst ? '' : 'disabled'}>
        <i class="ti ti-calendar-due"></i> Ver próximos vencimientos
      </button>
      <div id="prev-zonas"></div>`}`;

    if (inst && seccion === 'preventivo') renderZonas(inst);
  ...
  /**
   * Lista simple de qué está por vencer o ya venció, ordenada de más a menos
   * urgente. Sirve para planificar antes de salir a caminar el edificio, sin
   * necesidad de escanear nada: reutiliza el mismo cálculo que arma las zonas.
   */
  async function verVencimientos() {
    mostrar('prev-pant-vencen');
    const c = $('prev-vencen-cuerpo');
    c.innerHTML = '<div class="prev-hint">Buscando…</div>';
    try {
      const r = await api('/semaforo?institucion=' + encodeURIComponent(cfg.institucion) +
        '&responsable=' + encodeURIComponent(FILTRO()));
      const lista = r.atencion || [];

      const cabecera = `
        <div class="prev-titulo">Próximos vencimientos</div>
        <div class="prev-hint">${lista.length
          ? lista.length + ' equipo(s) por atender, del más urgente al menos urgente.'
          : 'Nada por atender ahora: todo lo que tenés a cargo está al día.'}</div>`;

      if (!lista.length) { c.innerHTML = cabecera; return; }

      c.innerHTML = cabecera + lista.map((e) => {
        const dias = e.dias_restantes;
        const texto = dias === null || dias === undefined ? 'sin ningún preventivo todavía'
          : dias < 0 ? `venció hace ${Math.abs(dias)} día${Math.abs(dias) === 1 ? '' : 's'}`
          : dias === 0 ? 'vence hoy'
          : `vence en ${dias} día${dias === 1 ? '' : 's'}`;
        const color = e.semaforo === 'rojo' ? '#c0392b'
          : e.semaforo === 'amarillo' ? '#B8791A' : '#8a93a0';
        return `<div class="prev-hito" style="border-left-color:${color}"
                     onclick="PREV.verHistorialSolo('${esc(e.equipo_id)}')">
          <div class="prev-hito-cab">
            <span class="prev-chip" style="background:${color}22;color:${color}">
              ${esc(texto)}</span>
            <span class="prev-hito-fecha">${esc(e.equipo_id)}</span>
          </div>
          <div class="prev-hito-tit">${esc([e.marca, e.tipo].filter(Boolean).join(' · '))}</div>
          <div class="prev-hito-det">${esc([e.lugar, e.piso ? 'piso ' + e.piso : '']
            .filter(Boolean).join(' · '))}</div>
        </div>`;
      }).join('');
    } catch (e) {
  ...
  /** Abre el historial de un equipo sin pasar por el flujo de escaneo. */
  function verHistorialSolo(equipoId) { verHistorial(equipoId); }
  ...
  <div class="prev-pant" id="prev-pant-vencen"><div id="prev-vencen-cuerpo"></div>
  ...
  window.PREV = {
    ...
    verVencimientos, verHistorialSolo,
