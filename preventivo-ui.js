...
(function () {
  ...
  async function resolverCodigo(texto) {
    ...
    if (eq) {
      ...
      if (modoEscaneo === 'trabajo') return abrirTrabajo(eq);
  ...
  function renderTrabajo() {
    ...
    $('prev-trabajo-cuerpo').innerHTML = `
      ...
      <div class="prev-hint">¿Hay que retirar el equipo? Eso se pide aparte, desde
        <b>Ver todo lo que se le hizo → Este equipo hay que reemplazarlo</b>: así lo
        aprueba el referente y queda dado de baja de verdad.</div>
  ...
  async function pedirBaja(equipoId) {
  ...
  function renderBaja() {
    ...
    $('prev-baja-cuerpo').innerHTML = `
      ...
      <div class="prev-card prev-warn">
        <b>Esto lo decide el referente</b>
        <p>El equipo sigue activo en el sistema hasta que se apruebe. No dejes de
           hacerle el preventivo mientras tanto.</p>
      </div>

      <label class="prev-lbl">¿Por qué hay que reemplazarlo?</label>
      <textarea id="prev-baja-motivo" class="prev-input" rows="4"
                placeholder="Ej: el compresor perdió el 40% de capacidad y no vale la reparación"></textarea>

      <button class="prev-btn prev-grande prev-btn-baja" onclick="PREV.guardarBaja()">
        <i class="ti ti-alert-triangle"></i> Enviar pedido de reemplazo
      </button>
      <div class="prev-hint" style="text-align:center">Firma: ${esc(FIRMA())}</div>`;
  }
  ...
  async function guardarBaja() {
    ...
    if (motivo.length < 8) { aviso('Contá brevemente por qué hay que reemplazarlo'); return; }
    ...
    try {
      ...
      await api('/cambio', {
        ...
        body: {
          ...
          rol: ROL,
          motivo,
          campos: { estado_equipo: 'fuera_servicio', motivo_baja: motivo },
        },
      });
      vibrar(30);
      aviso('Pedido enviado. Queda esperando al referente.');
      inicio();
    } catch (e) {
  ...
  async function verHistorial(equipoId) {
    ...
    try {
      ...
      c.innerHTML = `
        ...
        <button class="prev-btn prev-sec prev-btn-baja" onclick="PREV.pedirBaja('${esc(eq.equipo_id)}')">
          <i class="ti ti-alert-triangle"></i> Este equipo hay que reemplazarlo
        </button>`}
  ...
  window.PREV = {
    ...
    tipoTrabajo, elegirTicket, guardarTrabajo, verHistorial,
