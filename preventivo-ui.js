...
(function () {
  ...
  const RESPONSABLE = () => (ROL === 'oficial' ? 'oficial' : (EMPRESA() || '').toLowerCase());
      ...
      for (const reg of COLA.slice()) {
        try {
          ...
          if (e.status && e.status >= 400 && e.status < 500 && e.status !== 429) {
            // Error permanente (equipo inexistente, payload inválido): reintentar
            // eternamente no lo va a arreglar y trabaría toda la cola detrás.
            reg._error = e.message;
  ...
  async function subirRegistro(reg) {
    ...
    for (const f of (reg.fotos_pendientes || [])) {
      const r = await api('/foto', {
        method: 'POST',
        body: { base64: f.base64, nombre: f.nombre, mime: 'image/jpeg', institucion: reg.institucion_id },
      });
      urls.push(r.url);
    }
    const cuerpo = Object.assign({}, reg, { fotos: (reg.fotos || []).concat(urls) });
    delete cuerpo.fotos_pendientes;
    delete cuerpo._error;
    delete cuerpo._intentos;
    return api('/registro', { method: 'POST', body: cuerpo });
  }
    ...
    if (!TECNICO()) {
      cont.innerHTML = `
        <div class="prev-card prev-warn">
          <b>Falta tu nombre</b>
          <p>Antes de registrar preventivos, cargá tu nombre. Queda guardado y no
             hace falta escribirlo de nuevo.</p>
  ...
  function abrirAlta(id) {
    ...
    const tiposAlta = TIPOS.filter((t) => t.tipo_id !== 'base_general');
  ...
  function altaTipoCambio() {
    ...
    const pideRef = (t && t.campos || []).some(
      (c) => c.dispara && c.dispara.ref === 'ref_consumo_a');
    $('prev-a-extra').innerHTML = pideRef ? `
      <label class="prev-lbl">Consumo nominal del compresor (chapa del equipo)</label>
      <input id="prev-a-ref" class="prev-input" type="number" inputmode="decimal"
             step="0.1" placeholder="Amperes">
      <div class="prev-hint">Se carga una sola vez. Después solo medís el consumo real.</div>` : '';
  }
  ...
  async function guardarAlta() {
    ...
    const cuerpo = {
      ...
      ref_consumo_a: ref ? Number(ref.value) || null : null,
      usuario: FIRMA(),
    };
  ...
  function campoHtml(c) {
    ...
    if (c.tipo_campo === 'calculado') {
      return `<div class="prev-campo prev-calc" id="prev-c-${esc(c.campo_id)}">
        <span>${esc(c.etiqueta)}</span>
        <b id="prev-v-${esc(c.campo_id)}">—</b></div>`;
    }
    if (c.tipo_campo === 'numero') {
      return `<div class="prev-campo">
        ...
        <input class="prev-input" type="number" inputmode="decimal" step="0.1"
               value="${v == null ? '' : esc(v)}"
               oninput="PREV.set('${esc(c.campo_id)}', this.value === '' ? null : Number(this.value))">
      ...
      return `<div class="prev-campo">
        ...
        <div class="prev-ops">
          <button class="prev-op ${v === true ? 'on' : ''}"
                  onclick="PREV.set('${esc(c.campo_id)}', true)">Sí</button>
          <button class="prev-op ${v === false ? 'on mal' : ''}"
                  onclick="PREV.set('${esc(c.campo_id)}', false)">No</button>
        </div></div>`;
    }
      ...
      return `<div class="prev-campo">
        ...
        <div class="prev-ops">${c.opciones.map((o) =>
          `<button class="prev-op ${v === o.v ? 'on' : ''}"
                   onclick="PREV.set('${esc(c.campo_id)}', '${esc(o.v)}')">${esc(o.t)}</button>`).join('')}
        </div></div>`;
    }
    return '';
  }

  function set(campoId, valor) {
  ...
  function todoConforme() {
    for (const c of checklistActual) {
      ...
      if (c.tipo_campo === 'si_no') respuestas[c.campo_id] = true;
      else if (c.tipo_campo === 'opciones' && c.opciones.length) respuestas[c.campo_id] = c.opciones[0].v;
    }
    renderCampos();
    evaluarEnVivo();
  ...
  function evaluarEnVivo() {
    const vals = Object.assign({}, respuestas);
    for (const c of checklistActual) {
      if (c.tipo_campo !== 'calculado' || !c.formula) continue;
      const v = evaluarFormula(c.formula, vals);
      if (v !== null) vals[c.campo_id] = v;
      const el = $('prev-v-' + c.campo_id);
  ...
  async function guardarRegistro() {
    const faltan = checklistActual
      ...
      .filter((c) => respuestas[c.campo_id] === undefined || respuestas[c.campo_id] === null ||
    ...
    try {
      ...
      reg.origen = 'cola';
      await encolar(reg);
      aviso('No se pudo enviar: quedó en la cola');
      volverAEscanear();
    } finally {
  ...
  window.PREV = {
    ...
    altaTipoCambio, guardarAlta, todoConforme, set, guardarRegistro,
