  ...
  function abrirAlta(id) {
    ...
    $('prev-alta-cuerpo').innerHTML = `
      ...
      <select id="prev-a-padre" class="prev-input">
        ...
        ${(() => {
          // Primero los que naturalmente alimentan a otros, después el resto.
          // Bloquear el resto sería peor: si un tipo quedó mal cargado, el
          // técnico no tendría forma de vincularlo y quedaría trabado.
          const orden = { generacion: 0, distribucion: 1, tratamiento_aire: 2 };
          return INVENTARIO
            .filter((e) => e.equipo_id !== equipoActual.equipo_id)
            .slice()
            .sort((a, b) => (orden[a.funcion] ?? 9) - (orden[b.funcion] ?? 9) ||
                            String(a.equipo_id).localeCompare(String(b.equipo_id)))
            .map((e) => `<option value="${esc(e.equipo_id)}">${esc(e.equipo_id)}${
              e.funcion === 'generacion' ? ' ·' : ' ·'} ${esc(e.lugar || '')}</option>`)
            .join('');
        })()}
      </select>
